    ...
    class GestionConnexion:
        ...
        def connexion_personnel_pin(self, client,personnel, reset=True):
            ...
            return client.post("/connexion-personnel-pin", data=dict(
                login_select=personnel.login,
                hiddencode=personnel.password
            ), follow_redirects=True)
        
        def connexion_apprentis(self, client, formation, apprenti):
            Apprenti.reset_nbr_essais_connexion(apprenti["login"])
            return client.post(f"/connexion-apprentis/{formation['intitule']}/{apprenti['login']}", data=dict(
                login=apprenti["login"],
                password=apprenti["password"]
            ), follow_redirects=True)
