...
def d_apprenti():
    ...
    return {
        "d_formation": formation,
        "d_apprenti": ApprentiTest()
    }
...
def d_fiche():
    ...
    class FicheTest:
        def __init__(self):
            ...
            self.id_fiche = FicheIntervention.assigner_fiche_dummy_eleve(apprenti.login, self.id_personnel, self.date_demande, self.nom_du_demandeur, self.localisation, self.description_demande, self.degre_urgence, self.couleur_intervention, self.nom_intervenant, self.prenom_intervenant, self.id_cours)
            self.numero = FicheIntervention.get_fiche_par_id_fiche(self.id_fiche).numero
            
    return {
        "d_cour": cour,
        "d_apprenti": apprenti,
        "d_educ_admin": educ_admin,
        "d_fiche": FicheTest()
    }
