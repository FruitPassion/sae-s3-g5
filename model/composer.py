...
def get_composer_presentation(id_fiche=1):
    """
    Permet de recuperer toutes les catégorie d'une fiche

    :return: liste de dictionnaires
    """
    return convert_to_dict(compo.query.filter_by(id_fiche=id_fiche).with_entities(compo.id_element, compo.picto,
...
def get_composer_categorie(id_fiche=1):
    ...
    return convert_to_dict(compo.query.filter_by(
        id_fiche=id_fiche).with_entities(compo.id_element, compo.picto, compo.text, compo.taille_texte, compo.police,
...
def get_composer_non_categorie(id_fiche=1):
    """
    Permet tout les elements d'une fiche associés à une catégorie

    :return: liste de dictionnaires
    """
    return convert_to_dict(compo.query.filter_by(id_fiche=id_fiche).with_entities(
        compo.id_element, compo.picto, compo.text, compo.taille_texte, compo.police, compo.audio, compo.police,
        compo.couleur, compo.couleur_fond, compo.niveau, compo.position_elem, compo.ordre_saisie_focus).join(
        elem).filter(elem.type != "categorie").all())
